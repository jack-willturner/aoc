def add(prog, i, modes):
    result_loc         = prog[i+3]
    operand0, operand1 = prog[i+1], prog[i+2]
    prog[result_loc]   = (prog[operand0] if modes[-1]=='0' else operand0) + (prog[operand1] if modes[-2]=='0' else operand1)
    return prog, i+4

def mul(prog, i, modes):
    result_loc         = prog[i+3]
    operand0, operand1 = prog[i+1], prog[i+2]
    prog[result_loc]   = (prog[operand0] if modes[-1]=='0' else operand0) * (prog[operand1] if modes[-2]=='0' else operand1)
    return prog, i+4

def save(prog, i, modes=None):
    prog[prog[i+1]] = inp
    return prog, i+2

def out(prog, i, modes=None):
    global diagnostic_code
    diagnostic_code = prog[prog[i+1]]
    return prog, i+2

def jmpt(prog, i, modes=[]):
    opr0 = prog[prog[i+1]] if modes[-1] == '0' else prog[i+1]
    opr1 = prog[prog[i+2]] if modes[-2] == '0' else prog[i+2]
    if opr0 != 0:
        i = opr1
    return prog, i+3

def jmpf(prog, i, modes=[]):
    opr0 = prog[prog[i+1]] if modes[-1] == '0' else prog[i+1]
    opr1 = prog[prog[i+2]] if modes[-2] == '0' else prog[i+2]
    if opr0 == 0:
        i = opr1
...
def lt(prog, i, modes=[]):
    ...
    prog[prog[i+3]] = 1 if opr0 < opr1 else 0
    return prog, i+4
...
def execute(prog):
    pos = 0
    while pos < len(prog):
        opcode = str(prog[pos])[-2:]
        if opcode == '99':
            break
        opcode = '0'+opcode if (len(opcode) == 1) else opcode
        modes = (['0','0','0'] + [i for i in str(prog[pos])[:-2]])[-3:]

        prog, pos = opcodes[opcode](prog, pos, modes)
