class IntcodeComputer():
    ...
    def save(self, modes=[]):
        #print('SAVE: ', modes)
        if modes[-1] == '0':
            #print(' 0: ', self.input[self.input_pointer])
            self.prog[self.prog[self.ip+1]] = self.input[self.input_pointer]
        elif modes[-1] == '1':
            #print(' 1: ', self.input[self.input_pointer])
    ...
    def out(self, modes=[]):
        #print(self.prog[self.ip], self.prog[self.ip+1], self.relative_base)
        #print(prog)

        #print(self.prog[self.ip+1]+ self.relative_base)
        self.diagnostic_code = self.access(modes[-1], self.ip+1)
        print('\tDIAGNOSTIC CODE: ', self.diagnostic_code)
        self.ip += 2
...
a = IntcodeComputer(input=[2],prog=prog)
a.execute()
print(a.diagnostic_code)
